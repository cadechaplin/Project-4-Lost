  ...
  setup() {
    ...
    async function loadLeaflet() {
      ...
      try {
        ...
        // Create custom marker icon without using require
        const customIcon = L.value.icon({
          iconUrl:
        ...
        // Set as default icon
        L.value.Marker.prototype.options.icon = customIcon;
      } catch (error) {
    ...
    async function calculateRoute() {
      ...
      // Initialize routeInfo with default values
      routeInfo.value = {
        osmDistance: "0 km",
        osmDuration: "0 mins",
        aStarDistance: "0 km",
      ...
      try {
        ...
        // Update OSRM part of routeInfo
        routeInfo.value.osmDistance = osrmRoute.routes[0].legs[0].distance.text;
        routeInfo.value.osmDuration = osrmRoute.routes[0].legs[0].duration.text;

        // Calculate bounds - make much larger to ensure we get enough street data
        const bounds = {
          south: Math.min(startPoint.value.lat, endPoint.value.lat) - 0.05,
          north: Math.max(startPoint.value.lat, endPoint.value.lat) + 0.05,
          west: Math.min(startPoint.value.lng, endPoint.value.lng) - 0.05,
          east: Math.max(startPoint.value.lng, endPoint.value.lng) + 0.05,
        };

        console.log("Using bounds for A* algorithm:", bounds);

        // Calculate A* route with these bounds
        const aStarResult = await calculateAStarPath(
          startPoint.value,
          endPoint.value,
          bounds,
          { gridSize: 500 } // Use a much higher grid size
        );
        ...
        if (aStarResult.routes && aStarResult.routes[0]) {
          ...
          // Update A* part of routeInfo
          routeInfo.value.aStarDistance =
            aStarResult.routes[0].legs[0].distance.text;
          routeInfo.value.nodesExplored =
            aStarResult.routes[0].nodesExplored || 0;
        }

        console.log("A* Path:", aStarPath.value);
        console.log("Nodes explored:", routeInfo.value.nodesExplored);
      } catch (error) {
        console.error("Error calculating route:", error);
        // Don't alter routeInfo here, it's already initialized
      } finally {
