...
const points = [
  { lat: 47.608246, lng: -122.332303 }, // Seattle downtown
  { lat: 47.603501, lng: -122.322514 } // Space Needle
];
...
async function showExploredNodes(exploredNodesList) {
  for (let i = 0; i < points.length - 1; i++) {
    ...
    const aStarResult = await calculateAStarPath(startPoint, endPoint, SEATTLE_BOUNDS);

    console.log(`A* result from point ${i + 1} to point ${i + 2}:`, aStarResult);
    ...
    if (aStarResult.routes && aStarResult.routes[0]) {
      // Handle the result based on its format
      const { decodePolyline } = await import("./services/geoUtils.js");
      const aStarPath = decodePolyline(aStarResult.routes[0].overview_polyline.points);

      console.log("A* Path points:", aStarPath.length);
    }
